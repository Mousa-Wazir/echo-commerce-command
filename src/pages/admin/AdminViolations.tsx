...
const AdminViolations = () => {
  ...
  const filteredViolations = violations.filter(violation => {
    const matchesSearch = violation.reporter.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         violation.reported.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         violation.productName.toLowerCase().includes(searchTerm.toLowerCase());
                      ...
                      <div className="flex space-x-2">
                        {violation.status === 'pending' && (
                          <>
                            <Button 
                              size="sm" 
                              onClick={() => handleAction(violation.id, 'investigate')}
                            ...
                            <Button 
                              size="sm" 
                              variant="outline"
                              onClick={() => handleAction(violation.id, 'dismiss')}
                            >
                              Dismiss
                            </Button>
                          </>
                        ...
                        {violation.status === 'investigating' && (
                          <>
                            <Button 
                              size="sm" 
                              onClick={() => handleAction(violation.id, 'resolve')}
                              className="bg-green-600 hover:bg-green-700"
                            >
                              Resolve
                            </Button>
                            <button 
                              onClick={() => handleAction(violation.id, 'suspend')}
                        ...
                        <button 
                          onClick={() => handleAction(violation.id, 'delete')}
