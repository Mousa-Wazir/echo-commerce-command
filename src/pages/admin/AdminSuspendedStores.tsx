...
const AdminSuspendedStores = () => {
  ...
  const filteredStores = suspendedStores.filter(store => {
    ...
    const matchesDuration = filterDuration === 'all' || 
                           (filterDuration === 'temporary' && store.status !== 'permanent') ||
                           (filterDuration === 'permanent' && store.status === 'permanent');
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-gray-900">Suspended Stores</h1>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search suspended stores..."
                value={searchTerm}
                ...
                {filteredStores.map((store) => (
                  <tr key={store.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {store.reactivationDate !== 'Never' && (
                        <div className="text-sm text-gray-500">Until: {store.reactivationDate}</div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {store.status === 'active_suspension' && store.duration !== 'Permanent' && (
                          <>
                            ...
                            <Button 
                              size="sm" 
                              variant="outline"
                              onClick={() => handleExtendSuspension(store.id)}
