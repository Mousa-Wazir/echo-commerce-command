...
const AdminAnalytics = () => {
  const revenueData = [
    { name: 'Electronics', value: 12345, percentage: 35 },
    { name: 'Fashion', value: 8901, percentage: 25 },
    { name: 'Home & Garden', value: 6789, percentage: 19 },
    { name: 'Sports', value: 4567, percentage: 13 },
    { name: 'Books', value: 2890, percentage: 8 }
  ];

  const sellerPerformanceData = [
    { seller: 'TechRentals Pro', sales: 24500, orders: 156, growth: 23 },
    { seller: 'PhotoGear Hub', sales: 18200, orders: 89, growth: 18 },
    { seller: 'GameZone', sales: 15800, orders: 134, growth: 15 },
    { seller: 'Fashion Corner', sales: 12300, orders: 67, growth: 12 },
    { seller: 'Home Essentials', sales: 9800, orders: 45, growth: 8 }
  ];
      ...
      <div className="space-y-4 md:space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
            ...
            <Button onClick={exportReport} className="flex items-center gap-2 text-sm md:text-base">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          <Card>
            ...
            <CardContent>
              <div className="text-xl md:text-2xl font-bold">$328,000</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base md:text-lg">Seller Performance</CardTitle>
              <CardDescription>Top performing sellers by sales volume</CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer config={chartConfig} className="h-[250px] md:h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base md:text-lg">Top Performing Categories</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Orders</th>
                    <th className="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Growth</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {revenueData.map((category) => (
                    <tr key={category.name}>
                      <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{category.name}</td>
                      <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">${category.value.toLocaleString()}</td>
                      <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{Math.floor(category.value / 50)}</td>
                      <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-green-600">+{category.percentage}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
